...
(defn add-to-trie
  ...
  [trie seqs]
  (if-not (next seqs)
    (add-next trie (peek (vec seqs)))
    (recur
     (update-in trie
                (pop (vec seqs))
                add-next (peek (vec seqs)))
     (next seqs))))
...
(defn add-to-trie2
  ...
  (if-not (next seqs)
    ...
    (recur
     ...
     (if-not (contains? (get-in trie (pop (vec seqs))) (last seqs))
...
(defn build-trie
  ...
  (loop [trie in-trie
    ...
    (if-not right-seqs
      (add-to-trie2 trie left-seqs)
      (recur (add-to-trie2 trie left-seqs)
